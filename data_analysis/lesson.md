...
### 5-12. 集計結果に対する絞り込みは、WHEREではなくHAVINGを使う
...
### 5-13. 演習: PV数の多いページpathを特定する
- 「path毎のaccess_logs集計」のクエリを編集し、下記の条件を付け加えましょう
    - pathが"/"ではない
    - PV数が30以上
- クエリを保存します

### 5-14. コラム: 実行順序は、FROM → JOIN → WHERE → GROUP BY → HAVING → ORDER → LIMITの順番
...
### 5-15. ワーク: GROUP BYにまつわるよくあるエラーを体験する
